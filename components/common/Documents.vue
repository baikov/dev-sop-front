...
<template>
  <div
    ...
    :class="isFull ? '' : 'md:grid-cols-2 lg:grid-cols-3'"
  >
    <UCard
      v-for="doc in docs"
      :key="doc.id"
    ...
    >
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <UIcon
            :name="fileIconMapping[doc.file.split('.').slice(-1)[0] as keyof typeof fileIconMapping]"
            class="mr-2 size-5 shrink-0"
          />
          <NuxtLink
            :to="`${config.public.mediaUrl}/${doc.file}`"
